    ...
    <div class="page-wrapper mt-0 ">
        <div class="card  bg-dark text-white">
            <img class="w-100" src="{{ asset('ui-images/1.png') }}" alt="Card image"
                style="height: 800px; filter: blur(10px); object-fit: cover;">
            ...
            <div class="border wtext wborder p-3">
                <form action="{{route('admin.profile.update')}}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input id="password" type="password" class="form-control border-0 shadow @error('password') is-invalid @enderror" name="password">
                        @error('password')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input id="phone" type="text" class="form-control border-0 shadow @error('phone') is-invalid @enderror" name="phone" value="{{ old('phone', $user->phone) }}" autofocus>
                        @error('phone')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group row mb-1">
                        <div class="col-md-10 offset-md-1 text-center">
                            <small>If you want to go back to the profile page <a href="{{route('admin.profile.index')}}" title="Profile">click here</a></small>
                            <button type="submit" class="btn btn-sm wbtn border text-white fw-bold float-end">Update</button>
                            <a href="{{route('admin.profile.index')}}"  class="btn btn-sm wbtn border text-white fw-bold float-end"> Back</a>
                        </div>
                    </div>
                </form>
            </div>
