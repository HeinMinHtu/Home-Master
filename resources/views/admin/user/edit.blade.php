        ...
        <div class="col-xs-12 col-sm-8 col-md-6">
            <div class="border wtext wborder p-3">
                <form action="{{route('admin.user.profile.update',$user->id)}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <h3 class="text-center mb-4">User အကောင့်ပြုပြင်ရန်</h3>
                    <div class="mb-3">
                    ...
                    <div class="mb-3">
                        ...
                        <input id="password" type="password" class="form-control border-0 shadow @error('password') is-invalid @enderror" name="password" value="{{ old('email', $user->password) }}">
                        @error('password')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
